...
class event_v1_0:
    ...
    def Addofftestevent ( payload ):
        realPayload = json.loads(json.dumps(payload, indent=4, sort_keys=True, default=str))
        response = requests.post(f'http://dispatcher/event/v1_0/offTest', json=realPayload)
        return httpToJson(response)
...
class event_latest:
    ...
    def Addofftestevent ( payload ):
        realPayload = json.loads(json.dumps(payload, indent=4, sort_keys=True, default=str))
        response = requests.post(f'http://dispatcher/event/latest/offTest', json=realPayload)
        return httpToJson(response)
